...
```bash
.venv/bin/python -m pytest -q tests/        # 203 tests should pass
.venv/bin/ruff check src/ tests/ tools/ examples/
.venv/bin/mypy src/arbez/ tools/ tests/     # strict mode, 30 source files
...
| `mypy` | `pyproject.toml` `[tool.mypy]` | `strict = True`. `python_version = "3.10"`. 30 source files (src + tools + tests). |
