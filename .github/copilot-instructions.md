...
detector. v0.1.0 is the first stable public release; it ships the
trained Arbez model (S-002 + S-010), and from it onward the locked
public surfaces follow semver discipline.
...
From v0.1.0 — the first stable public release — the public API is
a stability contract under semver discipline. Breaking changes are
no longer routine: any break of a locked surface needs an explicit
ADR that argues why the change is worth a major-version bump, not
just that it is convenient.

Locked surfaces (changing these is a breaking change requiring
explicit ADR justification + a major-version rationale):
...
* `mypy src/arbez/ tools/ tests/` — strict mode over `src/`,
  `tools/`, and `tests/`; clean is required. **Examples are NOT
  in the CI mypy scope;** warnings there are advisory.
...
The test suite is large and grows with the SDK. Conventions:
...
* `arbez_benchmark3.py` is the canonical benchmark (S-089); don't
  add sibling scripts that duplicate its scope.
...
* **`release.yml`** (S-056, split per S-063, gate removed per
  S-074) — the publish pipeline for BOTH package indexes:
  * every push to `main` (and `workflow_dispatch`) → **TestPyPI**
    dev train (S-063). The build injects a PEP 440
    `<pyproject-version>.dev<run_number>` version at build time;
    the working tree never carries the dev suffix.
  * every `vX.Y.Z` tag push → **production PyPI** (S-074: any
    maintainer-tagged version publishes, including 0.0.x
    milestones — the old v0.1.0+ gate was removed).

  Three jobs: `build` (sdist + wheel via `python -m build`, plus
  the S-064 bundled-model manifest verification and the
  tag-matches-pyproject-version check), `publish-testpypi`, and
  `publish-pypi`. Both publish jobs authenticate via OIDC-based
  Trusted Publishing; auth chains documented in S-056 (TestPyPI)
  and S-063 (production PyPI).
  ...
  * **🚫 review-stop:** any PR that adds a `PYPI_API_TOKEN`
    ...
    credentials and shouldn't acquire them. This applies to
    BOTH the TestPyPI and the production-PyPI job.
  * **⚠️ must-fix-before-merge:** any PR that modifies the
    tag-matches-pyproject-version verification in the build
    job's "Compute target + version" step. That check is
    load-bearing — without it, a mistagged release would
    publish a misaligned version to production PyPI.
  * **⚠️ must-fix-before-merge:** any PR that renames the
    `testpypi` or `pypi` environment in `release.yml` without
    documenting the corresponding change to the publisher
    configuration. Environment names are part of the OIDC
    publisher registration on each index; the two have to
    match exactly or the handshake fails.
