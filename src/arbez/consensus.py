...
* **symbology** — most common Symbology in the group; ties go to the
  highest-scored detection's symbology.
...
def _aggregate_group(group: list[tuple[str, Detection]]) -> Detection:
    ...
    * **symbology**: most-common-value voting; tiebreak to the
      highest-scored member's symbology.
    ...
    * **extras**: ``voted_by`` (sorted tuple of unique engine names),
    ...
    sym_top, sym_top_n = sym_counts.most_common(1)[0]
    # If multiple symbologies tied at the top, prefer best_det's.
    symbology = (
        best_det.symbology
        if sym_counts[best_det.symbology] == sym_top_n
        else sym_top
    )
