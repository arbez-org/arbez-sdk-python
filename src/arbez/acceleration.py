...
def cuda_is_available() -> bool:
    ...
    ``ArbezEngine`` — the bundled default engine — consults this
    probe (via ``preferred_onnx_providers``) to decide between CUDA
    and CPU execution providers; install the ``[cuda]`` extra to
    enable the GPU path.
    """
...
def coreml_is_available() -> bool:
    ...
    Same role as :func:`cuda_is_available` on the Apple side:
    ``ArbezEngine`` uses it (via ``preferred_onnx_providers``) to pick
    the Core ML execution provider on Apple Silicon and falls through
    to CPU otherwise. Note that the built-in ``AppleVisionEngine``
...
def execution_providers() -> tuple[str, ...]:
    ...
    > CPU). This is the order ``ArbezEngine`` uses when picking which providers to enable on this host.
