...
``zxing``'s long-tail 1D coverage (Aztec, EAN-13, the 1D catch-all)
with no extra setup.
        ...
        if name not in installed:
            ...
            if name in known:
                raise EngineUnavailable(
                    ...
                    f"Installed: {sorted(installed)}. Install with "
                    f"`pip install 'arbez[{name.replace('_', '-')}]'`."
                )
            raise EngineUnavailable(
    ...
    * ``WeChatEngine`` — internally serialized with a per-instance lock.
    ...
    engine:
        ...
        * ``None`` (the default since S-075) — bare ``Scanner()`` runs
          ...
          coverage of long-tail 1D codes (Aztec, EAN-13, the
          1D catch-all) right from ``Scanner()``. The result's
    ...
    def __init__(
        ...
        *,
        ...
        # scanning. We still resolve it for ``repr()``-friendliness
        # (so a user can see which single-engine baseline they were on
        # before flipping to consensus), but the consensus path
        # ignores it. Skip the single-engine wiring entirely when
        # consensus="vote".
        if consensus == "vote":
            ...
            if not vote_names:
                raise EngineUnavailable(
                    ...
                    "filter. Install at least one of "
                    "`arbez[zxing]/[wechat]/[apple-vision]`, or omit "
                    "``engines=`` to use all installed engines."
                )
            ...
            # upfront. (Mirror check in run_consensus is defensive but
            # too late: a user constructing a Scanner that returns
            # nothing wants the error at construction time, not after
            # the first scan call.)
    ...
    def close(self) -> None:
        ...
        with self._engine_lock:
            if self._engine is not None:
                eng_close = getattr(self._engine, "close", None)
                if eng_close is not None:
                    try:
                        ...
                        _log.warning(
                            ...
                            "continuing teardown.",
                            type(self._engine).__name__, e,
                        )
                self._engine = None

        # Consensus engines (S-032) — same treatment for each voter.
        # Code-review fix (2026-05-17): hold ``_consensus_engines_lock``
        # for the iterate + None-assign. Pre-fix this was lock-free,
        # which races with concurrent ``_get_consensus_engines()``:
        # thread A enters close(), snapshots the dict reference, starts
        # iterating + closing each engine; thread B's
        # ``_get_consensus_engines`` already passed its
        # ``is not None`` fast-path and returns a reference to the
        # mid-teardown dict, then calls ``detect_and_decode`` on a
        # closed engine. Inside the lock, ``_get_consensus_engines``
        # either sees the populated dict (and may double-check it under
        # its own lock) or sees ``None`` and rebuilds; never the
        # half-closed state.
        with self._consensus_engines_lock:
            if self._consensus_engines is not None:
                for name, eng in list(self._consensus_engines.items()):
    ...
    def _get_engine(self) -> Engine:
        ...
        # Safe because ``self._engine`` is assigned exactly once and
        # never reassigned after. On free-threaded Python (3.13t/3.14t)
