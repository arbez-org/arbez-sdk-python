...
* ``arbez``        — ``min(8, max(2, cpu_count // 2))``. ONNX Runtime
                     already parallelizes a SINGLE inference via its
                     intra-op thread pool, so process-level workers
                     multiply against ORT's own threads — modest
                     fan-out (half the logical cores) avoids
                     oversubscription. Floor at 2 keeps a pipeline
                     going on small hosts; cap at 8 bounds the
                     aggregate working set (one ORT session's buffers
                     per in-flight scan).
...
def recommended_workers(engine: str = "auto") -> int:
    ...
    ----------
    engine:
        One of ``"auto"`` (default), ``"arbez"``, ``"zxing"``,
        ``"wechat"``, ``"apple_vision"``, or ``"consensus"``. ``"auto"``
        resolves the same engine :class:`Scanner` would pick on this
        host; ``"consensus"`` returns the per-image fan-out width for
    ...
    Heuristics
    ----------
    * ``arbez``        — ``min(8, max(2, cpu_count // 2))``. ORT's
                         intra-op pool parallelizes each inference
                         internally; modest process-level fan-out
    ...
    * ``consensus``    — ``len(installed_consensus_engines())``, min 1.
    ...
    if engine == "arbez":
        # ArbezEngine runs ONNX Runtime, which parallelizes WITHIN a
        # single inference via its intra-op thread pool — each scan
        # already fans out across cores. Stacking many process-level
        # workers on top multiplies thread counts (workers x ORT
        # intra-op threads) and thrashes the scheduler, so modest
        # parallelism wins:
        #
        # Formula: min(8, max(2, cpu_count // 2)).
        # * Half the LOGICAL cores leaves headroom for each session's
        #   intra-op threads.
        # * Floor at 2: keep a load/decode pipeline going even on
        #   small hosts.
        # * Ceiling at 8: bounds aggregate memory (one ORT session's
        #   working set per in-flight scan) on many-core machines.
