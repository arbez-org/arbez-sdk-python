...
def _probe_onnxruntime_or_raise() -> None:
    ...
    S-083 (generalises S-081): before this probe, a broken
...
class ArbezEngine:
    ...
    name:
        ...
        * Explicit string — overrides the arch-derived default AND
          survives the post-warmup arch-refresh. Named instances are
          distinguishable in ``Detection.engine``, so two same-arch
          ArbezEngine instances (e.g. bundled YOLOX-s + a user-trained
          YOLOX-s fine-tune) can vote together via
          :func:`arbez.consensus.run_consensus` with a dict of
          instances — without a distinct name they'd collide on
          ``"arbez"`` in the per-engine result keys. Note that
          ``Scanner(engines=...)`` accepts installed engine NAMES
          only, not engine instances.
    ...
    def __init__(
        ...
        *,
        ...
        # Lazy-loaded zxing-cpp module - if the install is intact
        ...
        with self._session_lock:
            ...
            try:
                ...
                _log.debug(
                    ...
                    "to detect-only mode (payload=None). zxing-cpp is a "
                    "core dependency; run "
                    "`pip install --force-reinstall arbez` to restore "
                    "decoding."
                )
    ...
    def _decode_detections(
        ...
        for d in raw_dets:
            ...
            out.append(Detection(
                ...
                score=d.score,
                payload=payload,
                # S-067/S-072: the instance name (arch-derived or the
                # explicit ``name=`` override) flows into
                # ``Detection.engine`` — a hardcoded "arbez" here would
                # make rtdetr/yolo11/named instances indistinguishable
                # in consensus result keys.
                engine=self.name,
                polygon=polygon,
                extras=extras,
            ))
