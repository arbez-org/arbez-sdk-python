...
def _probe_pyobjc_or_raise() -> None:
    ...
    S-081 (issue #43): before this probe, the first missing pyobjc
        ...
        *,
        ...
        path_input_fast_path:
            **S-080.** When ``True`` (default), ``detect_and_decode``
            ...
            Set ``False`` to force the PIL path — useful for byte-
            perfect parity comparisons against pre-S-080 behaviour
            or for inputs that pass the bench3 / S-049 PIL-format
        ...
        # S-081 (issue #43): probe the pyobjc dependency chain at
