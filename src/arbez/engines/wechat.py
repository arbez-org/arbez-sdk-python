...
def _probe_opencv_or_raise() -> None:
    ...
    S-083 (generalises S-081, issue #43): before this probe, the first
...
class WeChatEngine:
    ...
    # object. Callers needing parallelism must construct one
    # WeChatEngine per thread (e.g. via ``threading.local``); the
    # benchmark in examples/ demonstrates the pattern. See S-018.
    thread_safety: ThreadSafety = "per-thread"

    def __init__(self) -> None:
        # S-083 (generalises S-081, issue #43): probe ``cv2`` +
    ...
    def close(self) -> None:
        ...
        the detector on next call. Per S-038's per-thread thread_safety contract, in a
        ThreadPoolExecutor setup callers should construct + close one WeChatEngine per worker
        thread.
        """
        with self._lock:
            self._detector = None
