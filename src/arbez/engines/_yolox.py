...
def preprocess(
    ...
    # Clamp to >= 1 px per side: extreme aspect ratios (e.g. a 2x4000
    # strip scales the short side to 0.32 px, which ``int()`` floors
    # to 0) would otherwise feed PIL a zero dimension and raise a raw
    # ``ValueError`` from ``resize``.
