...
importing this module IS what triggers that load.
...
def _build_format_table() -> tuple[
    ...
    """Build the lookup tables once at import time.

    Kept inside a function so that ``import arbez.engines.zxing`` raises a clear error at module
    load if zxing-cpp isn't installed — instead of mysteriously failing in ``detect_and_decode``.
    """
    try:
        import zxingcpp as _z
    except ImportError as e:
        raise EngineUnavailable(
            "ZXingEngine requires zxing-cpp. Install with "
    ...
    # S-082 (issue #44): the GS1 DataBar family has SEVEN distinct
    ...
    # StackedOmni decode fell through ``_translate``'s "unknown matrix
    # → drop" arm and the SDK returned zero detections for a render
    # zxing-cpp DIRECT had decoded fine. Map every variant we can
    # find in the running zxing-cpp build to ``Symbology.GS1_DATABAR``
