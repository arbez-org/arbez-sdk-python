...
def test_warmup_actually_pre_warms_first_scan() -> None:
    ...
    Wall-clock pairs on shared CI runners are noisy — scheduler
    preemption can inflate any SINGLE scan by an order of magnitude
    (a scheduled run failed on exactly that). A warmup regression is
    systematic: it inflates the first scan on EVERY iteration; noise
    is not. So we take five independent (first, second) pairs — fresh
    Scanner + warmup() each iteration, like the original single-shot
    measurement — and assert on the MINIMUM ratio: one clean pair is
    enough to prove warmup moved the first-scan cost off the hot path.
    """
    ...
    for _ in range(5):
        ...
        # The 0.5 ms floor protects against timer noise when scans
        # are already <1 ms.
        ratio = first_ms / max(second_ms, 0.5)
        if ratio < best_ratio:
            best_ratio = ratio
            best_pair = (first_ms, second_ms)
    ...
    assert best_ratio < 3.0, (
        f"warmup() didn't move enough first-scan cost in any of 5 runs: "
        f"best first={best_pair[0]:.2f}ms second={best_pair[1]:.2f}ms "
        f"ratio={best_ratio:.1f}x"
    )
