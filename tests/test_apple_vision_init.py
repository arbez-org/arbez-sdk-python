...
This file exercises only the constructor's pyobjc probe (S-081, issue
#43), which has to behave correctly REGARDLESS of whether pyobjc is
installed:
...
def test_init_raises_engine_unavailable_when_pyobjc_core_missing(
    ...
    """S-081 (#43): missing ``pyobjc-core`` (the ``objc`` module) must
...
def test_init_raises_engine_unavailable_when_vision_framework_missing(
    ...
    """S-081 (#43): missing ``pyobjc-framework-Vision`` (the ``Vision``
...
def test_init_raises_engine_unavailable_when_quartz_framework_missing(
    ...
    """S-081 (#43): missing ``pyobjc-framework-Quartz`` (the ``Quartz``
