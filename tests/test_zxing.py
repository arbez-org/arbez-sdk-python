...
# ── S-082 (issue #44): DataBar variant inverse-map coverage ─────────────
...
_DATABAR_VARIANT_NAMES = (
    ...
    "DataBarStkOmni",   # RSS Expanded Stacked
...
def test_s082_every_databar_variant_maps_to_gs1_databar(variant_name: str) -> None:
    """S-082 (#44): pre-fix, only ``BarcodeFormat.DataBar`` (union bit,
...
def test_s082_translate_surfaces_databar_omni_detection() -> None:
    """S-082 (#44) end-to-end: a mocked zxingcpp Result with
    ...
    assert det is not None, (
        ...
        "the inverse map regression (issue #44) is back"
    )
