...
# construction cost out of the per-specimen budget. WeChat needs the
# opencv-contrib build (``cv2.wechat_qrcode``) and its constructor probes
# for it — gate construction like _VISION so a missing contrib build
# SKIPS the WeChat cases instead of erroring the whole module at
# collection time.
...
@pytest.mark.skipif(not WECHAT_AVAILABLE, reason="WeChat needs opencv-contrib (cv2.wechat_qrcode)")
...
def test_consensus_no_payload_conflicts(spec: Specimen) -> None:
    ...
    wechat_payloads: set[str | None] = (
        {d.payload for d in _WECHAT.detect_and_decode(spec.image)}
        if _WECHAT is not None else set()
    )
...
def test_consensus_covers_every_clean_qr() -> None:
    ...
    for spec in _QR_CORPUS:
        ...
        wechat_hit = (
            bool(_WECHAT.detect_and_decode(spec.image))
            if _WECHAT is not None else False
        )
        vision_hit = (
