...
full-corpus run (PR #66's TEST-REPORT.md) surfaced several concerns:
...
**Context.** Issue #43 (filed from a downstream consumer that uses
...
* **Fixes #43** — the decoder gate (and any other fallback-chain
...
**Context.** PR #42 (closed 2026-05-18 the same day it opened):
...
The CI failure on PR #42 was incidental (onnxruntime 1.24.3 +
...
the `actions/setup-python` 5→6 bump was a clean merge in PR #41).

---

## S-083 — `WeChatEngine` and `ArbezEngine` probe their deps at init, raising `EngineUnavailable` (2026-05-18)

**Context.** S-081 (issue #43) fixed the init-time-contract leak for
...
papercuts (S-081 / S-082's adjacent #44 / S-083) were surfaced by
...
**Context.** Issue #44 (filed from a downstream decoder-gate
...
* **Fixes #44** — a downstream decoder-gate consumer
...
**Integration note.** Both #43 (S-081) and #44 (S-082) were
...
7. **`DECISIONS.md` referenced "S-061" 4 times** (in S-073's
   ...
   reserved for PR #16's bench2 work that was retired unmerged
...
* `examples/arbez_benchmark2.py` — PR #16 (unmerged), the
...
features and divergent default behavior, was untenable. PR #16 had
...
1. **bench3 absorbs bench2's high-value features**:
   - URI corpus discovery + recursive walk via the new
     `examples/_corpus_source.py` module (lifted from PR #16
...
4. **PR #16 closed unmerged.** Its `_corpus_source.py` + 21
...
* **PR #16's S-061 vision still lands** — URI corpora, recursive
...
* If demand surfaces for the subprocess-per-cell isolation
  ...
  in PR #16's git history is preserved for reference.
...
The handoff to the upstream weight-export workflow (PR #23 + #26)
...
(GitHub PR #16). PR-B never opened. This work was tracked
internally as "S-061".
...
GitHub PR #16 was closed unmerged with a supersession note
pointing at PR #32 (which landed S-073).
...
* Past TestPyPI publishes (v0.0.34 yanked S-058; v0.0.35 yanked
  in this entry) remain browseable historically but are
  functionally dead — pip skips yanked versions on resolution.
...
This shipped to TestPyPI as part of the v0.0.34 wheel + sdist and
is publicly browseable at
``https://test.pypi.org/project/arbez/0.0.34/``.
...
notice. The leaky README stays browseable on the historical
release page but is functionally dead for new installs.
...
new high-severity alerts surfaced (#6 GHSA-cfh3-3jmp-rvhc PSD
OOB, #7 GHSA-whj4-6x5x-4v2j FITS GZIP decompression bomb, #8
GHSA-pwv6-vv43-88gr PSD tile OOB) — all in **Pillow image
...
* Pillow font integer overflow (S-048 #4): arbez doesn't load
  fonts.
* Pillow PDF parser DoS (S-048 #5): arbez doesn't parse PDFs.
...
* Pillow PSD / FITS CVEs (this entry, S-049 #6 / #7 / #8):
...
* **Step 1**: vulnerable code is the PSD ImageFile plugin
  (#6, #8) and the FITS ImageFile plugin (#7).
...
* **Step 3**: Category **3b** — easy to eliminate in arbez
  ...
  arbez API.
* Dismissed #6, #7, #8 with ``dismissed_reason=not_used``
...
1. **Cyclic import** between `scanner.py` and `parallelism.py`
   (CodeQL alerts #19, #22). The v0.0.20 fix made both directions
...
1. **Extract engine discovery to `src/arbez/_engine_discovery.py`.**
   ...
   is gone at every level — runtime, static analysis (CodeQL #19 +
   #22 close), and architecture (the discovery probes are a
...
distinguish single-engine vs consensus latency for billing /
...
* `Symbology` is locked from v0.1 (S-003); reordering breaks
  downstream code that depends on member order
...
1. **GitHub Actions billing was unblocked** during this session.
...
distortion, extreme lighting — which is what M-005 trains it on.
...
training pipeline (post-M-005 Stage 5), the SDK needs to load it
from somewhere in some
