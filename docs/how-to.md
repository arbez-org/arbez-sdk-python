...
parallel), `min(cpu_count, 8)` for Apple Vision on Apple Silicon
(`min(cpu_count, 16)` on Ultra chips), and half the CPUs (floored
at 2, capped at 8) for `arbez`. See
[API reference → arbez.parallelism](api-reference.md#arbezparallelism)
...
`recommended_workers("wechat")` returns three quarters of the
physical cores, floored at 2 and capped at 8 — the heavy detector
(per-instance ~80 MB memory footprint) is memory-bandwidth-bound,
and the measured efficiency cliff past 8 workers argues for
restraint.
...
### Multi-arch consensus (3 ArbezEngine instances voting)

`Scanner(engines=...)` accepts installed engine NAMES only — it
can't take pre-constructed instances (see
["Write your own engine"](#write-your-own-engine): instance
registration into the Scanner voter set is future work). To run a
single custom instance behind the Scanner wrapper, pass it via
`engine=`:
...
scanner = Scanner(engine=ArbezEngine(
    arch="rtdetr_v2_r18vd",
    model_path=Path("/opt/models/my_rtdetr.onnx"),
))
```

To vote across MULTIPLE custom instances, call
`arbez.consensus.run_consensus` directly with a
`{name: Engine}` dict. Each `ArbezEngine` derives a distinct
`engine.name` from its arch, which makes natural dict keys:
...
detections = run_consensus(
    ...
    {
        "arbez": ArbezEngine(),                       # bundled YOLOX-s
        "arbez-rtdetr": ArbezEngine(
            arch="rtdetr_v2_r18vd",
            model_path=Path("/opt/models/my_rtdetr.onnx"),
        ),
        "arbez-yolo11": ArbezEngine(
            arch="yolo11s",
            model_path=Path("/opt/models/my_yolo11s.onnx"),
        ),
        # ... plus any classical engine instances (ZXingEngine(),
        #     WeChatEngine(), AppleVisionEngine()) if you want them
    },
    min_votes=2,
)
```

`run_consensus` is the same voting machinery `Scanner` uses
internally — IoU clustering, majority vote, `engine="consensus"`
on every output Detection. It returns a plain detection tuple
(no `Result` wrapper).
...
`name="..."` to the second and key the dict accordingly:

```python
detections = run_consensus(
    coerce_to_pil("photo.jpg"),
    {
        "arbez": ArbezEngine(),                       # bundled
