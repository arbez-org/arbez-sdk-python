...
| `model` | `None` | Reserved for `Scanner`-level model paths. Today passing anything other than `None` raises `NotImplementedError`. For an actual user-supplied model use `ArbezEngine(model_path=...)` directly. |
...
| `iou_threshold` | `0.5` | When `consensus="vote"`, bbox IoU >= this value groups detections from different engines as the same physical barcode. Keyword-only. |
...
scanner = Scanner()                           # auto-pick per platform
...
print(f"{len(result)} codes in a {result.image_size[0]}x{result.image_size[1]} image "
      f"({result.timings_ms['engine']:.1f} ms)")
```
...
real confidence). Requires `[zxing]` extra.
...
confidence in `score`. Requires `[apple-vision]` extra. Raises
...
Hardware-acceleration probes. Today these are introspection — they
don't change any built-in engine's behavior. They're the groundwork
for `ArbezEngine`'s ONNX Runtime execution-provider auto-pick;
users can call them now to confirm a CUDA / Core ML setup without
...
if arbez.cuda_is_available():
    print("GPU will be engaged once ArbezEngine ships")
```
...
> **Note:** `AppleVisionEngine` already uses Apple's Neural Engine
> via the Vision framework — NOT through ONNX Runtime + Core ML EP.
> This probe is for the future `ArbezEngine` Core ML path.
