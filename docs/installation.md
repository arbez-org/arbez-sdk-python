...
- `numpy>=1.24`, `pillow>=10`, `onnxruntime>=1.18`, `zxing-cpp>=3.0`
...
to install on macOS x86_64; the wheel will resolve but engines that
expect Apple Silicon (Apple Vision, future Core ML path) won't work.
...
ZXing, WeChat, and the future `ArbezEngine` CPU path all work.
...
A baseline model is bundled in the wheel so `Scanner()` works fully
offline by default. Larger models are opt-in via the registry. See
[`DECISIONS.md`](../DECISIONS.md) for the hybrid distribution
architecture.
