...
architectures behind a single `Scanner` and let the consensus
mechanism vote across them. Each engine instance contributes its
detections; the consensus picks based on agreement (per
`docs/concepts.md` consensus rules).
...
scanner = Scanner(engines=[
    ArbezEngine(),  # default: bundled YOLOX-S, name="arbez"
    ArbezEngine(arch="rtdetr_v2_r18vd",
                model_path=Path("/models/my_rtdetr.onnx")),
    ArbezEngine(arch="yolo11s",
                model_path=Path("/models/my_yolo11.onnx")),
    # ... plus the non-Arbez engines if you want them in the consensus
])

result = scanner.scan("test.jpg")
```

Because `ArbezEngine.name` derives from arch by default, the three
instances have distinct names (`arbez`, `arbez-rtdetr`,
`arbez-yolo11`) and coexist cleanly in the consensus result keying.
