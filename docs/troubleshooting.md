...
> framework directly (NOT ONNX Runtime + Core ML EP). The probe is
> for future `ArbezEngine`.
...
1. **`[apple-vision]` extra not installed:**

   ```bash
   pip install 'arbez[apple-vision]'
   ```

   Pulls `pyobjc-framework-Vision` and `pyobjc-framework-Quartz`.
...
- **`pillow` versions.** arbez requires `pillow>=10`.
