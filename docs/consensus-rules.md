...
| `symbology` | `Counter(members' symbologies).most_common(1)[0]`. **Tie → take the highest-scored member's symbology.** |
| `payload` | If any non-None payloads exist in the cluster: `Counter(non-None payloads).most_common(1)[0]`. **Tie → highest-scored member's payload (if non-None), else Counter's first-encountered top.** If no non-None payloads: `None`. |
...
Payload tiebreak:
  payloads = ["A1", "B1"]
  Counter("A1", "B1").most_common(1)[0] = ("A1", 1)   # tied at count=1;
                                                       # Counter returns
                                                       # first-encountered
  top_payload, top_n = "A1", 1

  best_det = max(members, key=score)
           = (arbez, Detection(payload="A1", score=0.97))

  if best_det.payload is not None and payload_counts[best_det.payload] == top_n:
...
The consensus result is deterministic **except** in two named
tied-value corners. Both are rare in practice and observable only
when explicitly hit.
...
Effect: which detection becomes a cluster's seed can flip across
        ...
        symbology/payload tiebreaks can flip.
...
### N2 — Counter ties in symbology / payload aggregation

```
Where: Counter.most_common returns first-encountered among tied
       counts. "First-encountered" = order of insertion into the
       Counter = group iteration order = inherits N1.

Effect: per-cluster symbology and payload picks can flip when
        multiple values tie at the same count.
```

## Determinism guarantees (independent of N1 / N2)
...
- `symbology` when N2 fires on tied symbology counts.
- `payload` when N2 fires on tied payload counts.
