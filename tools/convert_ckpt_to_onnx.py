...
NOTE: ``tools/sync_bundled_model.py`` is the canonical bundling path
for updating the shipped model (staged-candidate download, metadata
cleaning via ``tools/clean_bundled_model.py``, provenance recorded in
``bundled_model.lock.json``). Use this converter only for the raw
ckpt -> ONNX step on checkpoints you trained yourself.
...
Security
--------
``torch.load`` runs with ``weights_only=False`` because Lightning
checkpoints carry pickled non-tensor state that ``weights_only=True``
rejects. Loading therefore EXECUTES pickle bytecode embedded in the
checkpoint file. Only run this script on checkpoints you trained
yourself. The sha256 pin above is the guard; overriding it (via the
``ARBEZ_CKPT_SHA256`` environment variable) prints a loud warning
before loading and shifts the pickle-execution risk to you.
...
# Override via the ARBEZ_CKPT_SHA256 environment variable when
# converting a newly trained checkpoint. Overriding triggers a loud
# warning in main() before torch.load runs: the load executes pickle
# from the checkpoint (see the Security section of the module
# docstring), so only point this script at checkpoints you trained
# yourself.
EXPECTED_CKPT_SHA256 = os.environ.get("ARBEZ_CKPT_SHA256", _PINNED_CKPT_SHA256)

SHA_PIN_OVERRIDDEN = EXPECTED_CKPT_SHA256 != _PINNED_CKPT_SHA256
...
MODEL_QR_MAP_50 = "0.833"
MODEL_OVERALL_MAP_50 = "0.370"

# Model architecture parameters - fixed by the YOLOX-s training config.
NUM_CLASSES = 14         # length of the detector's class list
INPUT_SIZE = 640
...
def load_state_dict_into_model(model: torch.nn.Module, ckpt_path: Path) -> None:
    ...
    module.
    """
    # SECURITY: weights_only=False is required - Lightning checkpoints
    # carry pickled non-tensor state (hyperparameters, callback state)
    # that weights_only=True rejects. The trade-off: torch.load
    # EXECUTES pickle bytecode from the file. The sha256 pin in
    # verify_source_hash() is the guard; never point this script at a
    # checkpoint you did not train yourself.
...
def export_to_onnx(model: torch.nn.Module, output_path: Path) -> None:
    ...
    Output format: same as Megvii's stock YOLOX-s export - (1, 8400, 19)
    where 19 = 4 bbox (cx,cy,w,h, in INPUT-PIXEL coords; YOLOX-s decodes
    internally during forward) + 1 objectness + 14 classes.
...
def main() -> int:
    ...
    if SHA_PIN_OVERRIDDEN:
        print()
        print("*** WARNING: checkpoint sha256 pin overridden via ARBEZ_CKPT_SHA256. ***")
        print("*** torch.load runs with weights_only=False: loading EXECUTES pickle ***")
        print("*** code embedded in the checkpoint file. Only run this script on    ***")
        print("*** checkpoints you trained yourself.                                ***")
        print()

    print(f"==> Building YOLOX-s architecture (depth=0.33, width=0.50, num_classes={NUM_CLASSES})")
