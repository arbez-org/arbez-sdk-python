...
### Fixed — `AppleVisionEngine` raises `EngineUnavailable` at init when pyobjc is missing (S-081, fixes #43)
...
After PR #42 (a dependabot `python-minor-patch` group bump) tried
...
### Fixed — `ZXingEngine` surfaces every GS1 DataBar variant zxing-cpp returns (S-082, fixes #44)
...
* Four ADR cross-references to phantom S-061 (number was
  reserved for retired-unmerged PR #16 per S-073). Added stub
...
benchmark features previously scoped to the never-merged PR #16
...
single-engine sweep) is untouched. PR #16 is closed unmerged —
...
public release). The 14-class M-005-C taxonomy (bundled since
v0.0.38) is the supported contract.
...
3. **`docs/bring-your-own-weights.md` (NEW)** — formal public
   ...
   M-005-C `Symbology` class-id ordering, the preprocessing
...
|---|---|---|
| 🟠 high | `GHSA-cfh3-3jmp-rvhc` (#6) | PSD parser unreachable via allow-list |
| 🟠 high | `GHSA-whj4-6x5x-4v2j` (#7) | FITS parser unreachable via allow-list |
| 🟠 high | `GHSA-pwv6-vv43-88gr` (#8) | PSD parser unreachable via allow-list |
